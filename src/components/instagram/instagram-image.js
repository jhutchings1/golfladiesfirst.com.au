...
export function InstagramImage({ item }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex overflow-hidden font-sans text-sm text-white break-words whitespace-pre-wrap transition duration-200 ease-in-out opacity-0 hover:opacity-100 hover:bg-transparent-black-3/4">
        {imgLoaded && (
          <div className="absolute top-0 right-0 mt-2 mr-1 pointer-events-none">
            <svg
              viewBox="0 0 20 20"
              fill="currentColor"
              className="w-5 h-5 text-transparent-white-3/4"
            >
